...
export function CreateBusinessWindow({showUsersSection = false, closeFunction}: {readonly showUsersSection?: boolean, readonly closeFunction?: ()=>void}){
    const {setWaiting, setError, clearMessage,setSuccess} = useContext(MessageContext);
    const [addingPeople, setAddingPeople] = useState<boolean>(false);
        ...
        <div className="w-full h-[calc(100%-3.5rem)] md:h-[calc(100%-4.5rem)] bg-black/40 absolute flex place-content-center items-center z-50 mt-[3.5rem] md:mt-[4.5rem]">
            ...
            <form className="max-w-200 max-h-95/100 bg-white overflow-y-auto p-4 rounded-lg flex flex-col items-center gap-2.5 m-2" onSubmit={createBusiness}>
                <div className="relative">
                    <button className="font-bold w-full text-right rounded-full absolute text-xl" type="button" onClick={()=>closeFunction?.()}>X</button>
...

                {
                    ...
                    <div className="w-full mt-2">
                        <div className="flex place-content-between mb-2">
                            ...
                            <button className="rounded-button px-3 py-2 text-white bg-(--main-color) text-xs" type="button" onClick={()=>setAddingPeople(true)}>Agregar persona</button>
                        </div>
